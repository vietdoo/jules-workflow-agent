...
export function StudioShell() {
  ...
  const approvePlan = async () => {
    if (!state?.session_name) return;
    try {
      await controlApi.approvePlan(CONVERSATION_ID, state.session_name);
      await hydrate(true);
    } catch (caught) {
  ...
  const sessionRows = useMemo(() => sessions.slice(0, 6), [sessions]);
    ...
    <main className="studio-shell">
      <aside className={`sidebar ${sidebarOpen ? "sidebar--open" : ""}`}>
        ...
        <nav className="nav-list" aria-label="Studio navigation">
          ...
          <button className="nav-item"><FileText size={17} />Local journals <span className="nav-count">{summary.events}</span></button>
          <button className="nav-item"><TerminalSquare size={17} />Harness logs</button>
        ...
        <div className="workspace-grid">
          ...
          <aside className="orchestration-rail">
            <section className="rail-section rail-section--agent">
              <div className="rail-heading"><span>ACTIVE AGENT</span><button aria-label="Open agents"><PanelRight size={16} /></button></div>
              ...
              <div className="agent-select-list">{agents.map((agent) => <button key={agent.agent_id} onClick={() => chooseAgent(agent)} className={`agent-option ${agent.agent_id === state?.active_agent_id ? "agent-option--selected" : ""}`}><span>{agent.icon || agent.display_name[0]}</span>{agent.display_name}{agent.agent_id === state?.active_agent_id && <Check size={15} />}</button>)}</div>
            ...
            <section className="rail-section">
              ...
              <div className="branch-line"><span>Branch</span><strong>{state?.selected_branch || selectedSource?.default_branch || "Not selected"}</strong></div>
            </section>
            ...
            <section className="rail-section">
              ...
              {sessionNeedsApproval && <button className="approval-button" onClick={approvePlan}><Check size={15} />Approve plan</button>}
